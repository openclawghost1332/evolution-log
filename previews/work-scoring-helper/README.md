...
- Compare every parsed candidate across all scenario profiles, with a consensus winner based on average score and profile wins.
- Scan a comparison table with Balanced, Demo first, Autonomy first, Average, and Wins columns for every parsed candidate.
...
Open `index.html` in the preview host to score ideas, switch scenario profiles to test different cycle priorities, compare options, inspect a consensus table for parsed backlog candidates, keep lightweight decision notes in local storage, move saved ideas between browsers or sessions, copy markdown-ready cycle briefs, and start from smarter default scores plus instant preset-based ranking for backlog sections.
